...
put_rules(http_msg *msg, kv **params, size_t param_num, const char *rule_id)
{
	...
	// Updated three parts锛� enabled status锛宻ql and action
	// 1. update sql: parse sql, set raw_sql, set rule_id, do not need deal connection. free origin sql data锛� 
	// 2. update enabled status: need to deal connection,  status changed will lead to connect/disconnect.
	// 3, update actions: need to deal connection锛寀pdate repub/table.
