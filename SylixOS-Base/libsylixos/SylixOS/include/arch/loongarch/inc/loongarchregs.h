...
/*
 * Read/write operations of the csr register
 */
#define __csrrd(reg)                    \
({                                      \
    unsigned int __res;                 \
...
#define __dcsrxchg(val, mask, reg)      \
({                                      \
    __asm__ __volatile__(               \
        "csrxchg %0, %1, %2          \n"\
        :                               \
...
#define __lvz_gcsrrd(reg)               \
({                                      \
})

#define __lvz_dgcsrrd(reg)              \
({                                      \
})

#define __lvz_gcsrwr(reg, value)        \
do {                                    \
} while (0)

#define __lvz_dgcsrwr(reg, value)       \
do {                                    \
} while (0)

#define __lvz_gcsrxchg(val, mask, reg)  \
({                                      \
})

#define __lvz_dgcsrxchg(val, mask, reg) \
({                                      \
})

#define __iocsrrd_b(reg)                \
({                                      \
    unsigned char __res;                \
    __asm__ __volatile__(               \
        "iocsrrd.b %0, %1            \n"\
...
/* CSR */
#define csr_readl(reg)              __csrrd(reg)
#define csr_readq(reg)              __dcsrrd(reg)
#define csr_writel(val, reg)        __csrwr(reg, val)
#define csr_writeq(val, reg)        __dcsrwr(reg, val)

#define csr_xchgl(val, reg)         __csrxchg(val, mask, reg)
#define csr_xchgq(val, mask, reg)   __dcsrxchg(val, mask, reg)

/* GCSR */
#define gcsr_readl(reg)             __lvz_gcsrrd(reg)
#define gcsr_readq(reg)             __lvz_dgcsrrd(reg)
#define gcsr_writel(val, reg)       __lvz_gcsrwr(reg, val)
#define gcsr_writeq(val, reg)       __lvz_dgcsrwr(reg, val)
#define gcsr_xchgl(val, mask, reg)  __lvz_gcsrxchg(val, mask, reg)
#define gcsr_xchgq(val, mask, reg)  __lvz_dgcsrxchg(val, mask, reg)

/* IOCSR */
#define iocsr_readb(reg)            __iocsrrd_b(reg)
#define iocsr_readl(reg)            __iocsrrd_w(reg)
#define iocsr_readq(reg)            __iocsrrd_d(reg)
#define iocsr_writeb(val, reg)      __iocsrwr_b(reg, val)
#define iocsr_writel(val, reg)      __iocsrwr_w(reg, val)
#define iocsr_writeq(val, reg)      __iocsrwr_d(reg, val)

/*
 *  Configure language
 */
#define _ULCAST_  (unsigned long)
#define _U64CAST_ (unsigned long long)
...
#define LOONGARCH_CSR_EENTRY        0xc                                  /*  Exception entry base addr  */
...
#define LOONGARCH_CSR_PGDL          0x19                                /*  Page table base address     */
                                                                        /*  when VA[47] = 0             */

#define LOONGARCH_CSR_PGDH          0x1a                                /*  Page table base address     */
                                                                        /*  when VA[47] = 1             */
...
#define LOONGARCH_CSR_ERRSAVE       0x95                                /*  KScratch for machine error  */
                                                                        /*  exception                   */
...
#define LOONGARCH_CSR_FWPC          0x380                               /*  instruction breakpoint conf */
#define LOONGARCH_CSR_FWPS          0x381                               /*  instruction breakpoint stat */
...
#define LOONGARCH_CSR_IB4ASID       0x3b3                               /*  inst break point 4 asi    d */
...
#define get_csr_cpuid()    csr_readl(LOONGARCH_CSR_CPUID)
#define read_csr_excode()  ((csr_readl(LOONGARCH_CSR_ESTAT) & CSR_ESTAT_EXC) >> CSR_ESTAT_EXC_SHIFT)

#define csr_any_send(addr, data, data_mask, cpu)                   \
({                                                                 \
    unsigned long long val = 0;                                    \
                                                                   \
    ...
    val |= ((unsigned long long)data << IOCSR_ANY_SEND_BUF_SHIFT); \
    __iocsrwr_d(val, LOONGARCH_IOCSR_ANY_SEND);                    \
})

#define write_csr_index(idx)                __dcsrxchg(idx, CSR_TLBIDX_IDXM, LOONGARCH_CSR_TLBIDX)
#define read_csr_pagesize()                 ((__csrrd(LOONGARCH_CSR_TLBIDX) & CSR_TLBIDX_SIZEM) >> \
                                             CSR_TLBIDX_SIZE)
#define write_csr_pagesize(size)            __dcsrxchg(size << CSR_TLBIDX_SIZE, \
                                                       CSR_TLBIDX_SIZEM,        \
                                                       LOONGARCH_CSR_TLBIDX)
#define read_csr_tlbrefill_pagesize()       ((csr_readl(LOONGARCH_CSR_TLBREHI) & CSR_TLBREHI_PS) >> \
                                                        CSR_TLBREHI_PS_SHIFT)
#define write_csr_tlbrefill_pagesize(size)  __dcsrxchg(size << CSR_TLBREHI_PS_SHIFT, \
                                                       CSR_TLBREHI_PS,               \
...
#define read_csr_entryhi()          csr_readl(LOONGARCH_CSR_TLBEHI)
#define write_csr_entryhi(val)      csr_writel(val, LOONGARCH_CSR_TLBEHI)
#define read_csr_entrylo0()         csr_readl(LOONGARCH_CSR_TLBELO0)
#define write_csr_entrylo0(val)     csr_writel(val, LOONGARCH_CSR_TLBELO0)
#define read_csr_entrylo1()         csr_readl(LOONGARCH_CSR_TLBELO1)
#define write_csr_entrylo1(val)     csr_writel(val, LOONGARCH_CSR_TLBELO1)
...
#define read_csr_prcfg1()           csr_readl(LOONGARCH_CSR_PRCFG1)
#define write_csr_prcfg1(val)       csr_writel(val, LOONGARCH_CSR_PRCFG1)
#define read_csr_prcfg2()           csr_readl(LOONGARCH_CSR_PRCFG2)
#define write_csr_prcfg2(val)       csr_writel(val, LOONGARCH_CSR_PRCFG2)
#define read_csr_prcfg3()           csr_readl(LOONGARCH_CSR_PRCFG3)
#define write_csr_prcfg3(val)       csr_writel(val, LOONGARCH_CSR_PRCFG3)
...
#define read_csr_impctl1()          csr_readl(LOONGARCH_CSR_IMPCTL1)
#define write_csr_impctl1(val)      csr_writel(val, LOONGARCH_CSR_IMPCTL1)
#define write_csr_impctl2(val)      csr_writel(val, LOONGARCH_CSR_IMPCTL2)

#define read_csr_perfctrl0()        csr_readl(LOONGARCH_CSR_PERFCTRL0)
#define read_csr_perfcntr0()        csr_readl(LOONGARCH_CSR_PERFCNTR0)
#define read_csr_perfctrl1()        csr_readl(LOONGARCH_CSR_PERFCTRL1)
#define read_csr_perfcntr1()        csr_readl(LOONGARCH_CSR_PERFCNTR1)
#define read_csr_perfctrl2()        csr_readl(LOONGARCH_CSR_PERFCTRL2)
#define read_csr_perfcntr2()        csr_readl(LOONGARCH_CSR_PERFCNTR2)
#define read_csr_perfctrl3()        csr_readl(LOONGARCH_CSR_PERFCTRL3)
#define read_csr_perfcntr3()        csr_readl(LOONGARCH_CSR_PERFCNTR3)
#define write_csr_perfctrl0(val)    csr_writel(val, LOONGARCH_CSR_PERFCTRL0)
#define write_csr_perfcntr0(val)    csr_writel(val, LOONGARCH_CSR_PERFCNTR0)
#define write_csr_perfctrl1(val)    csr_writel(val, LOONGARCH_CSR_PERFCTRL1)
#define write_csr_perfcntr1(val)    csr_writel(val, LOONGARCH_CSR_PERFCNTR1)
#define write_csr_perfctrl2(val)    csr_writel(val, LOONGARCH_CSR_PERFCTRL2)
#define write_csr_perfcntr2(val)    csr_writel(val, LOONGARCH_CSR_PERFCNTR2)
#define write_csr_perfctrl3(val)    csr_writel(val, LOONGARCH_CSR_PERFCTRL3)
#define write_csr_perfcntr3(val)    csr_writel(val, LOONGARCH_CSR_PERFCNTR3)

/* Guest related CSRS */
#define read_csr_gtlbc()            csr_readl(LOONGARCH_CSR_GTLBC)
#define write_csr_gtlbc(val)        csr_writel(val, LOONGARCH_CSR_GTLBC)
#define read_csr_trgp()             csr_readl(LOONGARCH_CSR_TRGP)
#define read_csr_gcfg()             csr_readl(LOONGARCH_CSR_GCFG)
#define write_csr_gcfg(val)         csr_writel(val, LOONGARCH_CSR_GCFG)
#define read_csr_gstat()            csr_readl(LOONGARCH_CSR_GSTAT)
#define write_csr_gstat(val)        csr_writel(val, LOONGARCH_CSR_GSTAT)
#define read_csr_gintc()            csr_readl(LOONGARCH_CSR_GINTC)
#define write_csr_gintc(val)        csr_writel(val, LOONGARCH_CSR_GINTC)
#define read_csr_gcntc()            csr_readl(LOONGARCH_CSR_GCNTC)
#define write_csr_gcntc(val)        csr_writel(val, LOONGARCH_CSR_GCNTC)
...
#define EXCCODE_TLBXI      6                                            /*  TLB Execution-Inhibit exc   */
